...
(function(_, $) {
	...
	TimedAttackSystem.prototype.playCircleGame = function() {
	    	...
	    	if(this._image.trim().length > 0 && this._xPosition > 0) {
				...
				if(this._xPosition > 0) {
					for(var i = 0; i < this._x.length; i++) {
                        ...
                        //SergeofBIBEK Edit: Draw target circle
                        
                        this._content.bitmap.drawRing(this._x[i], this._y[i], (this._radius * 0.3), "rgba(0,255,0,1)", (this._thickness/5));
	    ...
	    if(eval($.activateCondition) && this._notPressed) {
            ...
            //SergeofBIBEK Edit: Set a success range
            var percentage = this._xPosition/this._radius;
            var goal = 0.3;
            var variance = 0.1;
            var distanceFromGoal = Math.abs(percentage - goal);
            
            if (distanceFromGoal <= variance)
                {
                    this.setPower(1 - (distanceFromGoal / variance));
                }
            else
                {
                    this.setPower(0);
