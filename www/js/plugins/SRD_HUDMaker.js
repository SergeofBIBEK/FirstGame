...
Imported["SumRndmDde HUD Maker"] = 1.11;
...
HUDManager.refreshChoices = function() {
	...
	for(let i = 0; i < this._data.length; i++) {
		...
		result += "<option value=" + i + ">[" + _.pad(data.id) + "] " + data.type + " (" + value + ")</option>";
	}
...
HUDManager.onDelete = function() {
	...
	this._data = datas;
	if(this._sprites.length > 0) this._currentId = 0;
	else {
...
Sprite_HUDObject.prototype.updateCondition = function() {
	if(this._condition) {
		this._isVisible = !!this._isActive || $gameTemp.isManipulatingHud;
