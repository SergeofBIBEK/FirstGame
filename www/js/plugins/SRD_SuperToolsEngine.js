...
WindowManager.setupWindowHtml = function() {
	...
	} else if(cnstr === Scene_Battle) {
		alert("This feature is not available within battle!");
...
WindowManager.getTableHtml = function() {
	...
	const win = this._focusWindow.constructor.name;
...
WindowManager.getWindowList = function() {
	...
	this._scene._windowLayer.children.forEach(function(window) {
		if(window && window.isOpen() && window.visible) {
			if(!this._focusWindow) this._focusWindow = window;
			result += '<option value="' + window.constructor.name + '">' + window.constructor.name + '</option>';
		}
	}, this);					
	result += '</select>';
	return result;
...
Scene_Base.prototype.getEditWindow = function(name) {
	...
	for(let i = 0; i < children.length; i++) {
		...
		if(win && win.constructor.name === name) {
			return win;
		}
	}
};
...
Scene_Base.prototype.setEditWindow = function(name) {
	...
	for(let i = 0; i < children.length; i++) {
		...
		if(win && win.constructor.name === name) { 
			...
			this._editWindowName = win.constructor.name;
...
Scene_Base.prototype.initCustomWindowPositions = function() {
	...
	for(let i = 0; i < children.length; i++) {
		const win = children[i];
		if(win && $dataWindows[this.constructor.name][win.constructor.name]) {
